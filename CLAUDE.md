...
- **开放可见 + 动作按"能否替班"分类（2026-06 反转 §3.3 安全墙，不可从代码反推）**：这是**公司内部提效系统**，全员可信、无外部租户。为"看起来正规的权限"按角色藏行只制造摩擦（曾因此打断协作、被迫加"鸡生蛋池例外 + 404 兜底"补丁）。故：① **可见性全员开放**——`build*Scope` 对任何在册角色返回空 `where`（含只读 viewer），仅未知/缺失角色 fail-closed；列表/详情/导出/统计/徽章一律全量（徽章=**团队待办计数**，谁都能接手），敏感字段也都给。"工位聚焦"改由正交机制承担：角色落地页 + 队列/awaiting/需关注筛选 + 侧栏徽章。② **动作权限按"是否真需要权限且不能替班"切两类**：**仍锁 admin/PM**——确认项目/确认交付/标记·恢复·终止异常（管理决策、有问责）、删除项目=仅 admin、建/改项目头=admin/PM/销售（`projectOwnerRoles`）；**放开给全员在岗（除只读 viewer，=`staffRoles`，`lib/auth/action-roles.ts`）**——收样/实验全流程/质控/产出指标/生信建单与执行/多模态确认/测序交付登记，以及原"必须本人才能操作该条"的实例归属校验，全部可互相替班。操作类角色常量（`experimentManageRoles`/`bioinfoManageRoles`/`bioinfoCreateRoles`/`sampleReceiveRoles`/`sampleCreateRoles`/`metricsRecordRoles`）已统一指向 `staffRoles`，名为 manage/create 仅兼容历史调用点。③ **viewer 保持只读**（`canActAsStaff`/`ensure*Role(staffRoles)` 挡在外）。**被否决**：维持 §3.3 行级安全墙——对内部提效系统是纯摩擦、无真实安全收益。`build*Scope` 的 `userId` 参数已随之删除（不再按人过滤）。
- **创建接缝 = 队列驱动，靠"行级 pool 例外 + awaiting 列表筛选 + 工作台卡"打通（2026-06）**：状态机能流转，但"子实体还没被创建、要人手动建"的接缝处，下一棒角色的工作面（子实体列表）里没有可显示的东西，导致他看不到该建什么。三处接缝：①项目→样本=自动生成（已消解）；②样本 received→建实验任务（S2）；③实验 completed→建生信任务（S3）。修法（纯读侧、零状态机）：列表加 `awaiting=task`/`awaiting=bioinfo` 筛选(其行均合格)→行内"建子实体"按钮深链 `?sampleId=`/`?experimentTaskId=` 预填表单；指引入口指过去（**M2 删工作台后由工位侧栏角标承接**，原工作台卡时代顺带修了"待分析"原错指 `/projects?status=waiting_bioinfo`、`/samples?received=1` 原死参）。**关键坑（鸡生蛋）**：`buildSampleScope`/`buildExperimentTaskScope` 按"我相关"过滤，对"尚无子实体"的待建项恒假 → 实验员/分析员**看不到自己该建的队列**。故给二者 scope 并上对应 pool（实验员 += 待建任务样本池；分析员 += 待建生信任务池），与既有 `waiting_schedule`/`waiting_arrival` pool 同源。三处口径（scope/列表筛选/工作台计数）共用 `lib/auth/role-scope.ts` 的 `samplesAwaitingTaskWhere`/`tasksAwaitingBioinfoWhere` 一份 where 片段。执行接缝（实体已存在等下一动作）无需此处理，已被"可筛列表+行内动作"覆盖。**（2026-06 更新：可见性已全员开放，"给 scope 并 pool"的鸡生蛋例外随硬 scope 一并移除；`samplesAwaitingTaskWhere`/`tasksAwaitingBioinfoWhere` 仅保留为列表 `awaiting=` 筛选与工位徽章的队列口径。）**
