...
- **UI 交互模型 = 行内动作 + 容器整页 / 叶子抽屉（2026-06，详见 [ADR-0002](docs/adr/0002-详情整页化与查询参数抽屉弃用拦截路由.md)，后续模块必须遵循）**：列表行尾直接放当前 status+角色下的语义动作（主动作按钮 + 破坏性动作收 `⋯` 溢出菜单，弹 Dialog 完成）。**详情按实体性质分**：项目（容器 hub，含样本/任务/生信/交付/时间线）= **整页**（行/编号点击直达 `/projects/[id]`，**顶栏面包屑**回退——所有全页（详情 + 新建/编辑表单）用 `SetBreadcrumb` 经 context 把「段/编号/当前」送进顶栏 `HeaderTitle`（无则回退段标题），**不渲染在正文**）；实验任务/样本/生信任务（叶子工作条目）= **`?view=<id>` 查询参数抽屉**（在各自工位列表读 `?view` 渲染 `DetailSheet`，关闭只删 `view` 参数保留筛选，抽屉头「↗ 打开全页」升级到全页）；交叉链直接整页跳转、不叠跨实体浮层。**表单（创建/编辑）一律整页**（`/entity/new`、`/entity/[id]/edit`——表单是专注事务、非高频扫读，且 create==edit 应一致）。**已弃用拦截路由 + `@sheet` 平行槽 + `FormModal`**（连根拆除，原因见 ADR-0002）。WHY：核心用户**队列驱动**；**行内动作已覆盖快捷操作**（不必进详情即可干活），常驻侧栏 + 面包屑 + 抽屉保上下文——原"侧滑保列表上下文"痛点已消解；曾按传统 list→detail→action 实现过一版（每条 4 跳丢上下文）被推翻，**禁止**回退到"动作只能进详情页做"。实现：`partition*Actions`（包规则层 `getAvailable*Actions` 纯函数，可单测）+ `components/detail/` 共享件（`DetailSheet`/`ActionOverlay`/字段网格/时间线）+ 顶栏面包屑 `components/header-breadcrumb.tsx`；`ClickableRow` 整页传 `href`、抽屉传 `?view` href + `scroll={false}`；列表页脚统一 `components/list/list-pager.tsx`（勿再内联翻页块）；表单成功后**硬导航**到全页详情（整页刷新拿最新数据）。**动作浮层 surface 债（2026-06）**：「列表行动作=右抽屉」仅 `lab`/`intake` 经 `surface="sheet"` 铺到位，`projects`/`bioinfo`/`system-users` 的 confirm/reason 类动作仍走默认居中 Dialog（`ConfirmDialog`/`ReasonDialog` 未加 surface 管线）——低收益、勿当 bug 顺手改。**▲▼ 上/下一条**（Vercel Logs 同款抽屉增强）列为后续。
