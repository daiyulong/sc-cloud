...
- **大重构设计（2026-06，设计已定稿、待实现；v2 经 6 视角对抗式复审修订）**：`docs/重构设计-2026-06.md`——批次样本模型（项目→样本批次 YP→样本 样本名）+ 工位制 IA（导航收敛 5+1：项目/收样/实验/生信/经验库 +用户，删工作台/交付/样本顶层）+ 多模态识别（Vercel AI Gateway 录实验记录图片、人工确认）+ 进度式信息收集。实现按其 M1→M4 里程碑推进；落地后按 §10 规格同步清单回写本文件与规格说明书。
