...
- **创建接缝 = 队列驱动（2026-06）**：状态机能流转，但"子实体还没被创建、要人手动建"的接缝处，下一棒角色的工作面里没有可显示的东西。两处接缝（项目→样本已自动生成消解）：②样本 received→建实验任务、③实验 completed→建生信任务。修法（纯读侧、零状态机）：列表加 `awaiting=task`/`awaiting=bioinfo` 筛选 → 行内"建子实体"按钮深链 `?sampleId=`/`?experimentTaskId=` 预填表单；队列深度由工位侧栏角标承接。三处口径（列表 `awaiting=` 筛选 / 工位徽章 / awaiting 计数）共用 `lib/auth/role-scope.ts` 的 `samplesAwaitingTaskWhere`/`tasksAwaitingBioinfoWhere` 一份 where 片段。**注**：旧"给 scope 并 pool 绕开行级墙"的鸡生蛋补丁已随可见性全员开放移除（见 [ADR-0001](docs/adr/0001-可见性全员开放与动作按替班分类.md)）——scope 现全开放，awaiting 仅作筛选/徽章口径，不再是 scope 例外。
...
- **外包测序 = 测序交付登记，复用实验状态机、不新建流程（2026-06，老板澄清后定，不可从代码反推）**：上机测序**外包**——样本寄厂商测序，厂商交付物=**邮件+excel**、真实数据在**外部对象存储**（科创云/阿里云 OSS…），旧流程实验员收邮件转发给生信。**效率红线**：不强制把 excel 已有数据**逐行手敲**（否则比转发邮件更费事）。指标三处出处：活率/浓度/结团=内部自测（手录合理）、**测序量=厂商 excel**、**捕获细胞数/基因中位数=生信 cellranger 产出**（分析员录）。**建模**：外包测序贴合现有实验状态机——`完成实验`(in_progress→waiting_feedback)=送外包测序、`提交反馈`(→completed)=数据回，**不新建实体/状态**；只补 `SequencingDelivery`（**项目粒度**，承接审计指出的"最硬缺口"=系统原本无处放数据链接+厂商存证）：`storageUrl` **通用命名**（不绑科创云，以后换/加云厂商不改模型）+ excel 存证（复用 `lib/storage/local`，已泛化出 `saveDeliveryFile`）+ 备注。登记动作（实验员/PM/管理员，`canRegisterDelivery`=experimentManageRoles）取代"转发邮件"：贴链接+传 excel→落库+写操作日志+best-effort 通知生信组→项目详情/生信任务详情透传链接据此取数。**v1 defer**：excel 自动解析填测序量（step 2，待真实厂商 excel 列格式确认再做，避免猜列名）、多样本一次建任务、完整 N:M 交付 join（DeliveryBatchSample）、状态机插"已寄送/待数据返回"gating。**被否决**：①审计建议的完整 DeliveryBatch N:M 实体 + 松动 BioinfoTask 单 FK=对 v1 过重、前置太多不确定（列格式/跨项目交付/覆盖语义）；②给单样本建任务套"待建 tab"=把机械步骤包装得更显眼（先简后繁，应"要么多样本做对、要么自动掉")。**关键坑**：生信分析员看**待开始(未指派)**生信任务时，项目 scope（要求该项目下有自己负责的生信任务）对其恒假，`listProjectDeliveries` 会 404 拖垮整页→生信详情页 `.catch(()=>[])` 兜底（认领后即可见）。
