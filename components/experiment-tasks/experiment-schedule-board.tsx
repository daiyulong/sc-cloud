...
export function ExperimentScheduleBoard({
  ...
  // 弹窗状态
  const [fillDialogProjectId, setFillDialogProjectId] = React.useState<string | null>(null)
  const fillDialogOpen = fillDialogProjectId !== null
  ...
  return (
    <>
            ...
            {appointmentBatches.length === 0 ? (
              ...
              <div className="grid gap-3 xl:grid-cols-2">
                {appointmentBatches.map((batch) => {
                  const needsConfirm = !batch.projectNo || !batch.contractNo
                  return (
                    <div
                      key={batch.id}
                      className="flex flex-col rounded-md border bg-background p-3 sm:flex-row"
                    >
                      {/* 左侧：项目确认（固定宽度） */}
                      <div className="flex w-28 shrink-0 flex-col justify-center border-b border-r p-2 sm:border-b-0 sm:border-r">
                        {needsConfirm ? (
                          <Button
                            size="sm"
                            variant="outline"
                            className="w-full"
                            onClick={() => setFillDialogProjectId(batch.projectId)}
                          >
                            项目确认
                          </Button>
                        ) : (
                      ...
                      <div className="min-w-0 flex-1 p-3">
                        <div className="flex items-start justify-between gap-2">
                          <div className="min-w-0">
                            ...
                            <p className="mt-1 truncate text-xs text-muted-foreground">
                              {batch.projectNo ?? "未编号项目"} · {batch.customerOrg}
                            </p>
                          </div>
                        ...
                        <p className="mt-2 text-xs text-muted-foreground">
                          可预约样本 {batch.sampleCount} 个 · 到样日期{" "}
                        ...
                        {canCreate && (
                          <Button asChild size="sm" className="mt-3 w-full">
                            <Link href={newTaskHref(activeDate, batch.id)} scroll={false}>
                              ...
                              预约到{" "}
                              {formatDate(`${activeDate}T00:00:00`, {
                                month: "numeric",
                                day: "numeric",
                              })}
                            </Link>
      ...
      <FillContractNoDialog
        open={fillDialogOpen}
        onOpenChange={(next) => {
          if (!next) setFillDialogProjectId(null)
        }}
        projectId={fillDialogProjectId ?? ""}
        onConfirmed={() => setFillDialogProjectId(null)}
      />
    </>
  )
}
