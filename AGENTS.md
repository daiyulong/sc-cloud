...
**当前为实现中阶段**：Next.js 16 + Prisma + NextAuth + shadcn/ui 项目骨架已建立，已完成登录、主布局、工作台、项目与样本模块（API/UI/状态动作），并完成 UI 交互模型重构（行内动作 + 侧滑详情 + 深链全页，见下方设计决策）。实验任务/生信分析/交付/经验库四个模块尚未实现，须按该交互模型实现。
