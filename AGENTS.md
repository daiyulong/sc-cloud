...
- **工位列表 = 状态 Tab（ADR-0003）**：所有工位（`/lab`，`/bioinfo-tasks` `/intake` `/projects` 后续跟进）的列表控件只用状态 Tab（待办/进行中/已完成，单值，`?tab=todo|doing|done`，默认不写）表达"走到哪一步"。**禁止**再把 scope（我的/团队）和 status 塞进同一 Tab 控件——原 `?range=mine/pending/all` 的教训就是把这两个正交维度混一起。**「我的/团队」scope toggle 已在 2026-07-01 二次修订中整体移除**：内部全员可见的小团队工具，个人/团队视角切换的实际价值低于界面复杂度；负责人信息已在表格列可见；`?operatorId=` 深链接/API 过滤能力仍保留但不在 UI 暴露。**新工位接入本 ADR 时不要默认照搬 scope toggle**，先验证是否真有"按负责人查"的需求。排期视图与状态 Tab 解耦为 `?mode=schedule` 单独标志位，不混进列表 Tab。状态 → 桶映射集中在 `lib/workstation-buckets.ts`，不允许散落到 page.tsx。详见 ADR-0003。
