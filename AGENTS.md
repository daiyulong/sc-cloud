...
- **工位列表 = 两级 Tab（ADR-0003，2026-07-01 四次修订）**：第一级是模式切换（`/lab` 为例：任务/排期，`?mode=schedule` 独立标志位，`variant="default"` 填充胶囊样式）；第二级是状态 Tab（待办/进行中/已完成，单值，`?tab=todo|doing|done`，默认不写，`variant="line"` 下划线样式，只在"任务"模式下渲染）。两级视觉样式不同，读者一眼区分"这是模式"还是"模式内的子状态"——**不要**拉平成一排四项，也**不要**把"排期"这类不同工作方式硬套成状态 Tab 的一个选项。`components/list/list-tabs.tsx` 的 `variant` prop 承载这个层级差异。**禁止**再把 scope（我的/团队）和 status 塞进同一 Tab 控件——原 `?range=mine/pending/all` 的教训就是把这两个正交维度混一起。**「我的/团队」scope toggle 已在二次修订中整体移除**：内部全员可见的小团队工具，个人/团队视角切换的实际价值低于界面复杂度；负责人信息已在表格列可见；`?operatorId=` 深链接/API 过滤能力仍保留但不在 UI 暴露。**新工位接入本 ADR 时不要默认照搬 scope toggle**，先验证是否真有"按负责人查"的需求。状态 → 桶映射集中在 `lib/workstation-buckets.ts`，不允许散落到 page.tsx。详见 ADR-0003。
