...
- **工位列表 = 状态 Tab + scope toggle 正交化（ADR-0003）**：所有工位（`/lab` `/bioinfo-tasks` `/intake` `/projects`）的列表控件只用状态 Tab（待办/进行中/已完成，单值，`?tab=todo|doing|done`，默认不写）表达"走到哪一步"；scope 走单独的 Toggle（我的/团队，`?scope=mine|team`，角色驱动默认）。**禁止**再把 scope 和 status 塞进同一 Tab 控件。原 `?range=mine/pending/all` 已重塑为「状态 Tab + scope toggle」，与原"待排期"心智的排期视图解耦为 `?mode=schedule` 单独标志位。状态 → 桶映射集中在 `lib/workstation-buckets.ts`，不允许散落到 page.tsx。scope 与 status 永远正交，不让"待排期"混进列表 Tab。
